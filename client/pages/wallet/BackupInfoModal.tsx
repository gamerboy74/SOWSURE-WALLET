...
export const BackupInfoModal = React.memo(
  ({ backupInfo, onClose }: BackupInfoModalProps) => {
    ...
    const [copiedField, setCopiedField] = useState<
      "address" | "privateKey" | "mnemonic" | null
    >(null);

    const copyToClipboard = (
      text: string,
      field: "address" | "privateKey" | "mnemonic"
    ) => {
      navigator.clipboard
        .writeText(text)
        .then(() => {
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <button
                  onClick={() =>
                    copyToClipboard(backupInfo.privateKey, "privateKey")
                  }
                  className="absolute top-1/2 right-3 transform -translate-y-1/2 text-gray-500 hover:text-indigo-500 dark:hover:text-indigo-400 hover:scale-110 transition-all duration-200"
            ...
            {backupInfo.mnemonic && (
              <div>
                ...
                <div className="mt-1 relative">
                  ...
                  <button
                    onClick={() =>
                      copyToClipboard(backupInfo.mnemonic, "mnemonic")
                    }
                    className="absolute top-1/2 right-3 transform -translate-y-1/2 text-gray-500 hover:text-indigo-500 dark:hover:text-indigo-400 hover:scale-110 transition-all duration-200"
            ...
            <div className="space-y-4">
              <p className="text-sm text-red-600 dark:text-red-400 font-medium">
                ⚠️ WARNING: Store these details securely—they cannot be
                recovered if lost!
              </p>
              <button
                onClick={onClose}
