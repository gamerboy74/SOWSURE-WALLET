...
const Orders: React.FC = () => {
  ...
  const chatParams = useMemo(() => ({
    ...
    product: selectedOrder
      ? {
          ...
          type: (userRole === 'farmer' ? 'sell' : 'buy') as 'sell' | 'buy',
          farmer: { id: selectedOrder.farmer_id! },
          buyer: { id: selectedOrder.buyer_id! },
        }
      : null,
    disableChat: false,
  }), [userId, selectedOrder, userRole]);
    ...
    const getUserRole = async () => {
      try {
        ...
        const [farmerRes, buyerRes] = await Promise.all([
          supabase.from('farmers').select('id').eq('user_id', user.id).single(),
    ...
    try {
      ...
      const updatedOrders = await Promise.all(
        allProducts.map(async (product: ProductData) => {
          ...
          const statusMap = { '0': 'PENDING', '1': 'FUNDED', '2': 'IN_PROGRESS', '3': 'DELIVERED', '4': 'COMPLETED', '5': 'CANCELLED', '6': 'DISPUTED', '7': 'RESOLVED' };
          const mappedStatus = statusMap[contractDetails.status.status.toString()] || contract.status;
          ...
          if (mappedStatus !== 'PENDING') {
            user = await (async () => {
              if (userRole === 'buyer' && (!product.farmers?.name || !product.farmers?.id)) {
                const { data: farmer } = await supabase.from('farmers').select('name, profile_photo_url, id').eq('id', contract.farmer_id).single();
                return { name: farmer?.name || 'Unknown Farmer', type: 'Farmer', rating: 4.5, profilePhotoUrl: farmer?.profile_photo_url || null, id: farmer?.id || contract.farmer_id || '' };
              } else if (userRole === 'farmer' && (!product.buyers?.company_name || !product.buyers?.id)) {
                const { data: buyer } = await supabase.from('buyers').select('company_name, profile_photo_url, id').eq('id', contract.buyer_id).single();
                return { name: buyer?.company_name || 'Unknown Buyer', type: 'Buyer', rating: 4.5, profilePhotoUrl: buyer?.profile_photo_url || null, id: buyer?.id || contract.buyer_id || '' };
              }
              return {
  ...
  const handleExport = useCallback(() => {
    const csvContent = [
      ...
      ...filteredOrders.map((order) => [
        order.id, order.contract_id, order.status, order.product, order.quantity, order.price, order.location,
        order.user?.name || 'N/A', new Date(order.orderDate).toLocaleDateString(), new Date(order.deliveryDate).toLocaleDateString(),
      ]),
    ]
      .map((row) => row.join(','))
      .join('\n');
  ...
  const handleViewDetails = useCallback((orderId: number) => {
    if (userRole) navigate(`/${userRole}/orders/${orderId}`);
  }, [navigate, userRole]);
  ...
  const OrderCard = React.memo(({ order }: { order: Order }) => (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <span
          className={`px-2 py-1 text-xs font-medium rounded ${
            ...
            ['FUNDED', 'IN_PROGRESS'].includes(order.status) ? 'bg-blue-100 text-blue-700' :
            ['DELIVERED', 'COMPLETED', 'RESOLVED'].includes(order.status) ? 'bg-green-100 text-green-700' :
            ['DISPUTED', 'CANCELLED'].includes(order.status) ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'
          }`}
        >
          {order.status}
        </span>
      ...
      <div className="space-y-2 text-sm text-gray-600">
        <p className="flex items-center"><Package className="w-4 h-4 mr-2 text-teal-500" /> {order.quantity}</p>
        <p className="flex items-center"><MapPin className="w-4 h-4 mr-2 text-teal-500" /> {order.location}</p>
        <p className="flex items-center"><Calendar className="w-4 h-4 mr-2 text-teal-500" /> Ordered: {new Date(order.orderDate).toLocaleDateString()}</p>
        {order.status === 'DELIVERED' ? (
          <p className="flex items-center"><Truck className="w-4 h-4 mr-2 text-teal-500" /> Delivered: {new Date(order.deliveryDate).toLocaleDateString()}</p>
        ) : (
          <p className="flex items-center"><Calendar className="w-4 h-4 mr-2 text-teal-500" /> Delivery: {new Date(order.deliveryDate).toLocaleDateString()}</p>
        )}
      </div>
      {order.user ? (
        <div className="mt-4 pt-4 border-t border-gray-200 flex items-center">
          {order.user.profilePhotoUrl ? (
            <img src={order.user.profilePhotoUrl} alt={order.user.name} className="w-10 h-10 rounded-full mr-3" loading="lazy" />
          ) : (
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">{order.user.type} • ⭐ {order.user.rating}</p>
          </div>
        </div>
      ) : (
      ...
      <div className="mt-4 flex gap-2">
        ...
        >
          {chatLoading && selectedOrder?.id === order.id ? 'Opening...' : `Contact ${order.user?.type || 'Party'}`}
        </button>
      </div>
    </div>
  ));
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {chatError && <p className="fixed bottom-4 right-4 text-red-600 bg-red-100 p-4 rounded-lg">{chatError}</p>}
      </div>
    </div>
  );
};
