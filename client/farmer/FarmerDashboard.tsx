  ...
  const confirmSave = async () => {
    try {
      ...
      if (selectedImage && profile?.user_id) {
        ...
        if (profile?.profile_photo_url && !profile.profile_photo_url.startsWith('blob:')) {
          try {
            console.log("Original profile photo URL:", profile.profile_photo_url);
            const urlPath = profile.profile_photo_url.split('/storage/v1/object/public/farmer-documents/')[1];
            if (!urlPath) {
              console.warn("Could not extract path from URL:", profile.profile_photo_url);
            } else {
              const oldFilePath = urlPath;
              const deleteUrl = `${SUPABASE_URL}/storage/v1/object/farmer-documents/${oldFilePath}`;
              ...
              if (!deleteResponse.ok) {
                console.warn("Delete failed with status:", deleteResponse.status);
        ...
        const uploadUrl = `${SUPABASE_URL}/storage/v1/object/farmer-documents/${filePath}`;
        ...
        if (!uploadResponse.ok) {
          ...
          throw new Error(`Image upload failed: ${uploadResponse.statusText} - ${errorText}`);
        }

        // Set the new profilePhotoUrl only after successful upload
        profilePhotoUrl = `${SUPABASE_URL}/storage/v1/object/public/farmer-documents/${filePath}`;
      }

      // Step 3: Update the farmer profile in the database
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-6">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        {...register("land_size", {
                          ...
                          min: {
                            value: 0,
                            message: "Land size must be positive",
                          },
                        })}
                        className={`w-full mt-1 p-3 border rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 ${
                          errors.land_size
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        value={profile?.land_type || "Not set"}
                        disabled
